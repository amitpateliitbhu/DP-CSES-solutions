...
/*--------------------------------------------------------------------------------------------------*/
  ...
  for(int  i = 1 ; i <= x; i++){ //subarray size loop
    for(int j = 0 ; j+i<=x; j++){ // subarray starting loop
        // if(i == 1){
        //     dp[i][j] = arr[j];
        // }
        // else{
            int e = j + i - 1;
            lol g = (j+2 <= e)?dp[j+2][e] : 0;
            lol h = (j+1 <= e-1)? dp[j+1][e-1] : 0;
            lol o = (j <= e-2)? dp[j][e-2] : 0;
 
            dp[j][e] = max(arr[j] + min(g,h), arr[e] + min(h,o));
